...
pub fn test_main() {
    let list_of_test = [
        ...
        TestlistElem(
            ...
            String::from("test_framework_default"),
        ),
    ...
    for test in list_of_test {
        ...
        if test_result.failed == 0 {
            println!(
                "\x1b[32mall {} test(s) passed.\x1b[0m",
    ...
    if failed_count == 0 {
        println!(
            "All test finished, \x1b[32mall {} test(s) passed.\x1b[0m",
            passed_count
        );
    } else {
        if passed_count == 0 {
            println!(
                "All test finished, \x1b[31mall {} test(s) failed.\x1b[0m",
                failed_count
            );
        } else {
            println!(
                "All test finished, \x1b[31m{} test(s) failed.\x1b[0m",
                failed_count
            );
        }
    }
