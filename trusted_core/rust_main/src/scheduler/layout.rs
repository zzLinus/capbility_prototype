...
/*
    state to be stored onto mem when trapping
    including:
        1. general purpose reg x 32
        2. status register: align with boot/s_trap.S
        sstatus, sepc, stval, scause


    force this to be #[repr(C)], which promises
    #[repr(C)]
    struct {               fields are properly aligned
        a: usize,          in memory, a is stored at lower address than b
        b: usize           [in memory layout]
    }                       0x8 b
                            0x0 a
*/
#[repr(C)]
